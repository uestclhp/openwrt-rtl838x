				  ...
				  struct device *parent, bool strict)
{
	...
	pr_debug("%s() - looking at %pOF, has compatible prop\n", __func__, bus);
	...
	pr_debug("%s() - looking at %pOF, creating device\n", __func__, bus);
	dev = of_platform_device_create_pdata(bus, bus_id, platform_data, parent);
	if (!dev)
		pr_debug("Error getting device\n");
	if (!dev || !of_match_node(matches, bus))
		return 0;
	
	pr_debug("%s() - looking at %pOF, probing children\n", __func__, bus);
